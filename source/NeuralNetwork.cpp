...
void NeuralNetwork::loadWeights(string filename) {
  ...
  for(int i = 0; i < m_weightMatrices.size(); i++) 
  {
    for(int r = 0; r < m_weightMatrices.at(i)->getNumRows(); r++) 
    {
      for(int c = 0; c < m_weightMatrices.at(i)->getNumCols(); c++) 
      {
        m_weightMatrices.at(i)->setValue(r, c, temp.at(i).at(r).at(c));
      }
    }
  }
}